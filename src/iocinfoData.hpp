...
class Data {
 public:
   ...
   */
  inline void printLoadedLibraries() {
    std::cout << loadedLibraries.dump(1) << std::endl;
  }
 ...
 private:
   ...
   */
  void envMapToJson(nlohmann::json &j,
                    const std::map<std::string, std::string> &m);

  /**
   * @brief fetch information of loaded libraries from require and marshal them
   * into json
   * @param[in] loadedLibs json object to write to
   */
  inline void libVersionToJSON(nlohmann::json &loadedLibs) {
    loadedLibs.clear();
    foreachLoadedLib(Data::myLibHandler, &loadedLibs);
  };
   ...
   */
  static size_t myLibHandler(const char *name, const char *version,
